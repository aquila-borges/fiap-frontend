import { NextRequest } from "next/dist/server/web/spec-extension/request";
import { NextResponse } from "next/server";
import { JsonServerBankAccountRepository } from "@/infraestructure/JsonServerBankAccountRepository";
import { UpdateBankAccountUseCase } from "@/usecases/bankAccount/UpdateBankAccountUseCase";
import { GetBankAccountUseCase } from "@/usecases/bankAccount/GetBankAccountUseCase";

export async function GET(req: NextRequest, context: { params: { bankAccountId: string } | Promise<{ bankAccountId: string }> }) {
  try {
      const params = await context.params;
      const {bankAccountId} = params;

      if (!bankAccountId) {
        return NextResponse.json({ error: 'Bank account ID not provided' }, { status: 400 });
      }

      const repo = new JsonServerBankAccountRepository();

      const useCase = new GetBankAccountUseCase(repo);
      
      const result = await useCase.execute(bankAccountId);
      return NextResponse.json(result);
  } catch (error: any) {
      console.error('Error fetching bank account:', error);
      return NextResponse.json({ error: error.message ?? "Internal server error" }, { status: 500 });
  }
}

export async function PATCH(
  req: NextRequest,
  context: { params: { bankAccountId: string } | Promise<{ bankAccountId: string }> }
) {
  try {
    const params = await context.params;
    const {bankAccountId} = params;

    if (!bankAccountId) {
      return NextResponse.json({ error: 'Bank account ID not provided' }, { status: 400 });
    }
    const body = await req.json();

    const bankAccountRepo = new JsonServerBankAccountRepository();

    const updateUseCase = new UpdateBankAccountUseCase(bankAccountRepo);
    await updateUseCase.execute(bankAccountId, body);

    return NextResponse.json({ message: 'Bank account updated successfully' }, { status: 200 });
  } catch (error: any) {
    console.error('Error updating bank account:', error);
    return NextResponse.json({ error: error.message ?? "Internal server error" }, { status: 500 });
  }
}